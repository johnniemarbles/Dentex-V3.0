...
import { Calendar, Clock, CheckCircle, RefreshCw, UserPlus, MapPin, XCircle, Building, ArrowRight } from 'lucide-react';
...
export default function ClinicDashboard() {
  ...
  if (!clinic) return (
      <div className="flex flex-col items-center justify-center bg-slate-950 min-h-screen text-white p-6">
          <div className="w-16 h-16 bg-slate-900 rounded-2xl flex items-center justify-center mb-6 border border-slate-800">
             <Building className="w-8 h-8 text-slate-600" />
          </div>
          <h2 className="text-2xl font-bold mb-3">No Clinic Found</h2>
          <p className="text-slate-400 mb-8 text-center max-w-md">
            It looks like you haven't associated your account with a clinic yet. Claim your practice to access the dashboard.
          </p>
          
          <Link 
            to="/clinic-os/claim" 
            className="group flex items-center gap-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 px-8 rounded-xl transition-all shadow-lg shadow-emerald-900/20"
          >
            Claim Your Clinic <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
          </Link>
          
          <div className="mt-8 pt-8 border-t border-slate-800 text-center">
             <p className="text-xs text-slate-600 uppercase tracking-wider font-bold mb-2">Dev Shortcuts</p>
             <p className="text-xs text-slate-600">Ensure you are logged in via Supabase Auth first.</p>
          </div>
      </div>
  );
