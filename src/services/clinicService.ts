 ...
 * 4. STRICT: No Simulation Fallback. Real API or nothing.
 */
    ...
    if (results.length < 3) {
        ...
        try {
            if (GOOGLE_API_KEY) {
                ...
                // Note: Client-side fetch is subject to CORS. 
                // If it fails, we catch the error and do NOTHING (no simulation), per instructions.
                const response = await fetch(googleUrl);
                const data = await response.json();
                
                if (data.status === 'OK' && data.results) {
                   console.log(`✅ [Harvester] Google API Success: Found ${data.results.length} results.`);
                   ...
                   for (const place of data.results) {
                        ...
                        if (!exists) {
                            // Check DB again by google_place_id
            ...
            console.error("❌ [Harvester] Google API Failed (Likely CORS). Simulation is DISABLED.");
            // No fallback.
        }
    }
